...
int main(int argc, char *argv[])
{
    ...
    int bindcheck=bind(sockfd, (struct sockaddr *)&serverinfo, sizeof(serverinfo));
    if (bindcheck < 0){
    ...
    while(1){
        ...
        if(pid==0){
            ...
            sendcheck=send(findclientsocfd, "gotencrypt", 10, 0);
...
//            }
            for(i=0;i<charsread-1;i++){
                ...
                {
                    printf("otp_enc error: %s contains bad characters\n",argv[1]);
                    exit(1);
                }
                else if(readbuffer[i]==' '){
                    sendbuffer[i]='@';
                }else{
                    readint=(int)readbuffer[i];
                    keyint=(int)keybuffer[i];
                    sendint=(readint+keyint-130)%26;
                    sendbuffer[i]=(char)(sendint+65);
                }
//                if(sendbuffer[i]=='@') sendbuffer[i]=' ';
            }
